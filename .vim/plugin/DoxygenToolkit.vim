...
" add funciont name tag
if !exists("g:DoxygenToolkit_fnTag_pre")
  let g:DoxygenToolkit_fnTag_pre = "@fn:     "
endif
if !exists("g:DoxygenToolkit_fnTag_post")
  let g:DoxygenToolkit_fnTag_post = ""
endif
...
let s:ignoreForReturn = ['template', 'explicit', 'inline', 'static', 'virtual',  'const', 'volatile', 'struct']
...
function! <SID>DoxygenBlockFunc()
  ...
  if 0
    ...
    if(g:DoxygenToolkit_compactOneLineDoc =~ "yes" )
      let s:execCommand = "A "
    else
      let s:execCommand = "o"
    endif
...
function! <SID>DoxygenCommentFunc()
  ...
  if ( g:DoxygenToolkit_compactOneLineDoc =~ "yes" && l:doc.returns != "yes" && len( l:doc.params ) == 0)
    ...
    exec "normal O".strpart( s:startCommentTag, 0, 1 )
    exec "normal A".strpart( s:startCommentTag, 1 ).g:DoxygenToolkit_fnTag_pre.g:DoxygenToolkit_fnTag_post.l:lineBuffer
  else
    let s:compactOneLineDoc = "no"
    mark f
    exec "normal `i"
    let s:FunctionString = substitute(getline(line(".")), "\n", "", "g")
    exec "normal `f"
    let l:insertionMode = s:StartDocumentationBlock()
    exec "normal ".l:insertionMode.s:interCommentTag.g:DoxygenToolkit_fnTag_pre.g:DoxygenToolkit_fnTag_post.s:FunctionString
  endif
  if( l:doc.name != "None" )
