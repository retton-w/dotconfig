...
" Last Change: 16-May-2003 @ 19:18
" Created: Before 20-Jul-1999 (http://groups.yahoo.com/group/vim/message/6409)
" Requires: Vim-6.2, multvals.vim(3.2), genutils.vim(1.7)
" Version: 3.2.2
...
"
" TODO:
"   - ^G should also print the total number of files displayed.
...
if exists("loaded_selectbuf") || v:version < 602
  finish
endif
...
function! s:IncrementalUpdate()
  ...
  while MvIterHasNext('SelectBufUpdateAxns')
    ...
    " For 'c', 'm' or 'u', skip when the buffer is hidden and we don't [-2s]
    " show hidden buffers.
    elseif action =~ '[cum]' && ! s:showHidden && ! buflisted(bufNo)
      continue
    endif
...
function! s:DynUpdate(action, bufNum, ovrrdDelayDynUpdate)
  ...
  if s:ignoreNonFileBufs && (a:action != 'd' && a:action != 'w') &&
	\ getbufvar(bufNo, '&buftype') != ''
    return
  endif
